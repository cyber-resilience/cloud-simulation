...

{
  ...
  sub resp_file
  {
    ...
    if (-f "/data/files/$filename")
    {
      my $filecontents = '';
      open(FH, "/data/files/$filename");
      while (<FH>)
      {
        $filecontents .= $_;
      }
      close(FH);
      print($cgi->header('text/plain'), $filecontents);
      return;
    }
    ...
    if ($found)
    {
      ...
      #  Cache the file for faster response next time

      open(FH, ">/data/files/$filename");
      print(FH $db->get_file($filename));
      close(FH);
    }
    else
    {

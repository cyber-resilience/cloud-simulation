...
TWELVE_MINS_AGO=`date -d '-12 minutes' '+%F %T'`
...
if [ ! true ]; then
  ...
  echo      mergecap $PCAP_RAW/*.pcap -w $PCAP_2
  ...
  echo      mergecap $PCAP_RAW/*.pcap -w $PCAP_2_TMP
  /usr/sbin/mergecap $PCAP_RAW/*.pcap -w $PCAP_2_TMP
  echo      editcap -B "$TWO_MINS_AGO" $PCAP_2_TMP $PCAP_2
  /usr/sbin/editcap -B "$TWO_MINS_AGO" $PCAP_2_TMP $PCAP_2
fi
...
/usr/local/bin/matlab -nodisplay -nodesktop -r "run Main_demo.m" > $HOME/matlab.out 2>&1
cat $HOME/matlab.out | tail -5 | head -4 | ./create_html.pl > status.html

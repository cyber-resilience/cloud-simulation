...
for x in files:
	...
	#print nodes 
	#print inDegree
	F = open('/home/ubuntu/FFCAIDAOutDegree/' + x[40:(len(x)-5)] + '_degree_out.txt', 'w')
	for m in range(nodeCount):
		s = str(nodes[m]) + '\t' + str(outDegree[m]) + '\n'
		F.write(s)
	F.close()
	F = open('/home/ubuntu/FFCAIDAInDegree/' + x[40:(len(x)-5)] + '_degree_in.txt', 'w')
	for n in range(nodeCount):
		s = str(nodes[n]) + '\t' + str(inDegree[n]) + '\n'
		F.write(s)
	F.close()
	F = open('/home/ubuntu/FFCAIDADegree/' + x[40:(len(x)-5)] + '_degree.txt', 'w')

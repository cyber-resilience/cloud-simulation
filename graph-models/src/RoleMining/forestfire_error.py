...
output = open('FF-nccdc-error.txt', 'w')
#process run for each file
q = 0
for x in range(len(f_in_files)):
	output.write(str.strip(f_in_files[x])+'\n')
	print str.strip(f_in_files[x])
	...
	for k in range(len(inMaster)):
		error = (abs(new_freq_f_inDegree[k] - new_freq_inDegree[k]))
	...
	for k in range(len(outMaster)):
		error = (abs(new_freq_f_outDegree[k] - new_freq_outDegree[k]))
	...
	for k in range(len(degMaster)):
		error = (abs(new_freq_f_degree[k] - new_freq_degree[k]))
